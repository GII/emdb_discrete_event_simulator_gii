...
class BartenderSim:
    ...
    def reset_world(self):
        ...
        elif step == "at_prep_with_used_glass":
            self.robot_position = 0.0
            self.glass_in_left_hand = True
            self.bottle_in_right_hand = random.choice([True, False])
            self.glass["distance"] = 0.1
            self.glass["angle"] = 0.1
        ...
        # Client
        # Sample a random client id each reset to avoid deterministic client
        cid = int(self.rng.integers(1, 4))  # ids in [1,3]
        self.client_id = cid
    ...
    def pick_glass_policy(self):
        ...
        # Evitar recoger vaso inmediatamente después de colocarlo (detectar bucle)
        if self.last_policy_executed == "place_glass":
            return
        
        # En prep: solo recoger vaso si no está usado o si se va a preparar bebida
        if self.robot_position < 0.2 and self.glass_is_in_preparation_area():
            # No recoger vaso usado sin propósito claro
            if self.glass and self.glass["was_used"] and not self.bottle_in_right_hand:
                return
            self.glass_in_left_hand = True
            if self.glass:
        ...
        # En serving: recoger vaso para llevarlo (ya sea nuevo o usado)
    ...
    def place_glass_policy(self):
        ...
        # Evitar colocar y recoger repetidamente (detectar bucle)
        if self.last_policy_executed == "pick_glass":
            return
            
        if self.is_at_serving_table() and not self.glass["was_used"]:
            ...
            # Solo devolver vaso si está usado (retorno a posición original)
            if self.glass and self.glass["was_used"]:
                self.glass_in_left_hand = False
                self.glass["distance"] = self.original_glass_pos["distance"]
                self.glass["angle"] = self.original_glass_pos["angle"]
            # Si el vaso no está usado, no colocarlo en prep (evita bucle)
    ...
    def change_position_policy(self):
        """
        Toggle robot position between prep and serving tables.
        """
        if self.is_at_preparation_table():
            self.robot_position = 0.95  # Move to serving table
        else:
            self.robot_position = 0.0  # Move to preparation table
    ...
    def get_serve_the_drink_goal(self):
        ...
        if self.glass["state"] and not self.glass["was_used"]:
             reward = 1.0
    ...
    def get_return_the_glass_goal(self):
        ...
        Returns 1.0 when glass is back in prep area, has been used, and is not in hand.
        """
        reward = 0.0
        # Glass is in prep area if distance < 0.2
        glass_at_original = self.glass_is_in_preparation_area()
